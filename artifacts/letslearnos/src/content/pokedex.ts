// Offline character-guide metadata — entries #1..#1025.
// Names, identifiers, and type associations are factual metadata. Descriptive
// copy is generated below so this repository does not redistribute game text.
// Optional artwork is supplied locally; missing files use the neutral fallback.
...
type GuideEntryMetadata = Omit<PokedexEntry, "fact">;

const guideEntries: GuideEntryMetadata[] = [
...
function buildGuideFact(entry: GuideEntryMetadata): string {
  const themes = entry.types.map((type) => type.toLowerCase()).join(" and ");
  const noun = entry.types.length === 1 ? "theme" : "themes";
  return `This guide character is entry ${entry.id} and is associated with the ${themes} ${noun}.`;
}

export const pokedex: PokedexEntry[] = guideEntries.map((entry) => ({
  ...entry,
  fact: buildGuideFact(entry),
}));
